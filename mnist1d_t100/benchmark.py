...
class BootstrapSampler:
    """Fixed-size bootstrap resampling.

    Training set: ``train_size`` examples sampled *with replacement*.
    Validation and test sets: ``val_size`` and ``test_size`` examples drawn
    *without replacement* from the out-of-bag pool, so val and test are
    disjoint from each other and from the unique training examples.
    """

    def __init__(self, dataset, train_size, val_size, test_size):
        self.dataset = dataset
        self.n = len(dataset)
        self.train_size = train_size
        self.val_size = val_size
        self.test_size = test_size
        if val_size + test_size > self.n:
            raise ValueError(
                f"val_size + test_size ({val_size + test_size}) exceeds "
                f"dataset size ({self.n})"
            )

    def sample(self, rng):
        """Return (train, val, test) Subsets with independent bootstrap."""
        train_idx = rng.choice(self.n, size=self.train_size, replace=True)
        in_bag = set(train_idx.tolist())
        oob = np.array([i for i in range(self.n) if i not in in_bag])
        if len(oob) < self.val_size + self.test_size:
            raise RuntimeError(
                f"Out-of-bag pool ({len(oob)}) too small for "
                f"val+test ({self.val_size + self.test_size}); "
                f"increase dataset size or shrink train_size."
            )
        rng.shuffle(oob)

        val_idx  = oob[:self.val_size]
        test_idx = oob[self.val_size:self.val_size + self.test_size]

        train = Subset(self.dataset, train_idx.tolist())
        val   = Subset(self.dataset, val_idx.tolist())
        test  = Subset(self.dataset, test_idx.tolist())
...
class HyperparamSampler:
    ...
    ``lr_decay`` is the multiplicative factor gamma used by ``StepLR``
    (see ``Trainer``).  With probability ``p_zero_decay`` it is set to
    ``0.0``, signalling that no scheduler should be attached (constant
    learning rate).
    """
