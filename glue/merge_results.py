...
trial pool.  Mirror of code/cifar10/merge_results.py.
"""
...
parser.add_argument("--task", default="mrpc", choices=list(GLUE_TASKS),
                    help="GLUE task whose per-task output/<task>/ to merge")
args = parser.parse_args()
output_dir = os.path.join("output", args.task)
if not os.path.isdir(output_dir):
    raise SystemExit(f"No output directory {output_dir}/ — run the task first.")
print(f"Merging task '{args.task}' from {output_dir}/")
